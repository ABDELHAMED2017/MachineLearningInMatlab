...
function [y, MSE_time, neurons_all] = NeuralNetworkEarlyStop(adj_matrix, input, targets, num_input_nodes, num_hidden_nodes, num_output_nodes, ...
    ...
    MSE_time = zeros(num_itrs,1);
    
    % For the early stopping - save all neurons
    neurons_all = cell(num_itrs,1);
    ...
    for itr=1:num_itrs
        ...
        for j=jLast:total_num_nodes
           ...
           g(:,j) = input_struct(:,j); % Linear TF for output --> can be modified.
        end
        ...
        % 1. Output Units:
        for j=jLast:total_num_nodes
           dedx(:,j) = 2*(g(:,j) - targets); % Calculate error derivatives
        ...
        MSE_time(itr) = mean((g(:,total_num_nodes) - targets).^2);
        neurons_all{itr} = neurons; %save snapshots of the system at every iteration
